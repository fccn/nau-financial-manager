...
"""
    ...
    def do_POST(self):
        ...
        self.end_headers()
        self.wfile.write(self.response_as_xml.encode())
        self.wfile.close()


def __prepare_server() -> tuple:
    host = "127.0.0.1"
    setattr(settings, "TRANSACTION_PROCESSOR_URL", f"http://{host}:8009")
    server_address = (host, 8009)
    return server_address


def __start_server(server_class: HTTPServer, server_address: tuple, handler_class: BaseHTTPRequestHandler) -> tuple:
...
def run_mock_server(server_class=HTTPServer, handler_class=MockSageX3Server) -> tuple[HTTPServer, Thread]:
    server_address = __prepare_server()
    mock_server, thread = __start_server(
        server_class=server_class, handler_class=handler_class, server_address=server_address
    )
    return mock_server, thread
